...
const STORAGE_KEYS = {
  USER: 'weweight_user',
  RECORDS: 'weweight_records',
  PARTNER_RECORDS: 'weweight_partner_records',
  GOALS: 'weweight_goals',
  COUPONS: 'weweight_coupons'
};
...
const INITIAL_DATA = {
  user: {
    ...
    target_weight: 52.0,
    avatar_url: null,
    partner_id: "partner_yang",
    partner_name: "Yang",
    partner_avatar: null,
    invite_code: "OLIVIA-XP",
    privacy_show_weight: 1,
    privacy_show_progress: 1,
    privacy_show_chart: 1,
    p_show_weight: 1,
    p_show_progress: 1,
    p_show_chart: 1,
    points: 12
  ...
  records: [
    { id: 1, user_id: "user_olivia_yang", weight: 55.2, timestamp: new Date().toISOString() },
    { id: 2, user_id: "user_olivia_yang", weight: 55.5, timestamp: subDays(new Date(), 1).toISOString() },
    { id: 3, user_id: "user_olivia_yang", weight: 55.8, timestamp: subDays(new Date(), 2).toISOString() },
  ],
  partnerRecords: [
    { id: 101, user_id: "partner_yang", weight: 78.5, timestamp: new Date().toISOString() },
    { id: 102, user_id: "partner_yang", weight: 79.0, timestamp: subDays(new Date(), 1).toISOString() },
    { id: 103, user_id: "partner_yang", weight: 79.5, timestamp: subDays(new Date(), 2).toISOString() },
  ],
  goals: [
    { id: 1, user_id: "user_olivia_yang", title: "Reach 52kg", target_value: 52.0, type: "weight", deadline: null, completed: 0 },
    { id: 2, user_id: "user_olivia_yang", title: "Maintain for 7 days", target_value: 7, type: "date", deadline: null, completed: 0 },
  ],
  coupons: [
    { id: 1, user_id: "user_olivia_yang", title: "Fancy Dinner", cost: 100, description: "A high-quality low-carb dinner prepared by your partner." },
    { id: 2, user_id: "user_olivia_yang", title: "Full Body Massage", cost: 50, description: "30 minutes of relaxation after a long day." },
  ]
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const storedPartnerRecords = localStorage.getItem(STORAGE_KEYS.PARTNER_RECORDS);
    ...
    if (storedUser) {
      ...
      setPartnerRecords(JSON.parse(storedPartnerRecords || '[]'));
      setGoals(JSON.parse(storedGoals || '[]'));
      setCoupons(JSON.parse(storedCoupons || '[]'));
    } else {
      // First time setup
      setUser(INITIAL_DATA.user as User);
      setRecords(INITIAL_DATA.records as WeightRecord[]);
      setPartnerRecords(INITIAL_DATA.partnerRecords as WeightRecord[]);
      setGoals(INITIAL_DATA.goals as Goal[]);
      setCoupons(INITIAL_DATA.coupons as Coupon[]);
      
      localStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(INITIAL_DATA.user));
      localStorage.setItem(STORAGE_KEYS.RECORDS, JSON.stringify(INITIAL_DATA.records));
      localStorage.setItem(STORAGE_KEYS.PARTNER_RECORDS, JSON.stringify(INITIAL_DATA.partnerRecords));
      localStorage.setItem(STORAGE_KEYS.GOALS, JSON.stringify(INITIAL_DATA.goals));
      localStorage.setItem(STORAGE_KEYS.COUPONS, JSON.stringify(INITIAL_DATA.coupons));
    }
    setLoading(false);
  }, []);

  const handleAddRecord = (weight: number) => {
    if (!user) return;
  ...
  const handleConnectPartner = (code: string) => {
    if (!user) return;
    // Mock partner connection logic
    if (code === 'YANG-XP') {
      const updatedUser = { ...user, partner_id: 'partner_yang', partner_name: 'Yang' };
      setUser(updatedUser);
      localStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(updatedUser));
      
      // Also ensure partner records are visible
      setPartnerRecords(INITIAL_DATA.partnerRecords as WeightRecord[]);
      localStorage.setItem(STORAGE_KEYS.PARTNER_RECORDS, JSON.stringify(INITIAL_DATA.partnerRecords));
    } else {
      alert("Invalid code. Try 'YANG-XP'");
    }
  };
